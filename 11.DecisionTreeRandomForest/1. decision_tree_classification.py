...
healthinfo.physicalhealthbaddays.value_counts().sort_index()
...
dtc_params = {
 'decisiontreeclassifier__min_samples_leaf': np.arange(100, 1200, 100),
 'decisiontreeclassifier__max_depth': np.arange(2, 11)
}


rs = RandomizedSearchCV(pipe1, dtc_params, cv=5, scoring="roc_auc")
...
accuracy, sensitivity, specificity, precision = \
  ...
  skmet.recall_score(y_test.values.ravel(), pred2,  pos_label=0),\
